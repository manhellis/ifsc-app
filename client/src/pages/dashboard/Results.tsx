...
const Results = () => {
  ...
  useEffect(() => {
    // If there's no cid, redirect to the event page
    if (!cid) {
      navigate(`/dashboard/events/${id}`);
      return;
    }
    
    // If we have both id and cid, fetch the result
    if (id) {
      fetchResultData();
    }
  }, [id, cid]);
  ...
  const fetchResultData = async () => {
    ...
    try {
      // If we have both id and cid, use the combined endpoint
      if (id && cid) {
        // Use the API method that handles both id and cid
        const data = await resultsApi.fetchResultByIdAndCid(id, cid);
        if (data.result) {
          // The endpoint returns a single result object, not an array
        ...
        const data = await resultsApi.fetchResultById(id as string);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {results.length > 0 ? (
        ...
        !isLoading && !error && <div className="text-gray-500">No results found</div>
      )}
    </div>
  );
};
